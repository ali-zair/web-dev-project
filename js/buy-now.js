...
buyerDetailsForm.addEventListener('submit', async (event) => {
  ...
  // debugger;
  const item = items.find(item => item.id === parseInt(localStorage.getItem('itemID')));
  ...
  if (itemPrice * itemQuantity > user.balance) {
    ...
    if (itemQuantity > item.quantity) {
      ...
      items.splice(items.findIndex(item => item.id === parseInt(localStorage.getItem('itemID'))), 1, item);
      localStorage.items = JSON.stringify(items);
      const seller = users.find(user => user.itemsOwned && user.itemsOwned.includes(item.id));
      seller.bankAccount.balance += itemPrice * itemQuantity;
      users.splice(users.findIndex(user => user.uid === seller.uid), 1, seller);
      user.balance -= itemPrice * itemQuantity;
      user.itemsPurchased.push(purchaseDetails);
      users.splice(users.findIndex(user => user.uid === parseInt(localStorage.getItem('loggedInUser'))), 1, user);
